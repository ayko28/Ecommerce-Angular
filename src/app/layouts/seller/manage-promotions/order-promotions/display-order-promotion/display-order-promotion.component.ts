...
@Component({
  selector: 'app-display-order-promotion',
  templateUrl: './display-order-promotion.component.html',
  styleUrls: ['./display-order-promotion.component.scss']
})
export class DisplayOrderPromotionComponent {
 ...
 ngOnInit(){
  ...
  this.getProductList();   this.get_order_promotions(); this.getSellerList();

  this.getCategories();
  
  this.eventsSubscription = this.events.subscribe(() => this.get_order_promotions());//getProductList
}
...
get_order_promotions(){
  this.servPromoProduct.get_Promotion_Order_Seller(this.authServ.currentUserValue!.id).subscribe((res:any)=>{
